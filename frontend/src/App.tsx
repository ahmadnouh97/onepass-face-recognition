...
function LiveGate({ stationId, headers, notice, setNotice, reload }: { stationId: string; headers: Record<string, string>; notice: string; setNotice: (value: string) => void; reload: () => Promise<void> }) {
  ...
  async function start() {
    try {
      ...
      const ws = new WebSocket(wsUrl); socket.current = ws; frameInFlight.current = false;
      let timer: number | undefined;
      const scheduleFrame = (delay = 80) => { window.clearTimeout(timer); timer = window.setTimeout(captureFrame, delay); };
      const captureFrame = () => {
        ...
        const sourceWidth = video.current.videoWidth; const sourceHeight = video.current.videoHeight; const scale = Math.min(1, 640 / Math.max(sourceWidth, sourceHeight));
        ...
        canvas.current.toBlob((blob) => { if (!blob || ws.readyState !== WebSocket.OPEN) { if (ws.readyState === WebSocket.OPEN) scheduleFrame(); return; } frameInFlight.current = true; ws.send(blob); }, "image/jpeg", 0.76);
      };
      ws.onopen = () => { setStreaming(true); setNotice("Live recognition running. Processing one current frame at a time."); scheduleFrame(0); };
      ws.onmessage = (event) => { const message = JSON.parse(event.data); frameInFlight.current = false; if (message.type === "tracks") { setTracks(message.tracks); } if (message.type === "error") setNotice(message.detail); scheduleFrame(); };
